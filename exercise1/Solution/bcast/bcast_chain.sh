...
output_csv="../../../../../../bcast/Results/bcast_chain.csv"
...
# Run the benchmark test selecting the chain algorithm 2
for map in $maps
do
    ...
    do
        ...
        | tail -n 21 | awk -v cores="$cores" -v map="$map" '{printf "Chain,%s,%s,%s,%s\n",map,cores,$1,$2}' \
        | sed 's/,$//' >> $output_csv
    done
    ...
    --mca coll_tuned_bcast_algorithm 2 \
    osu_bcast -x 1000 -i $iter -f -z \
    | tail -n 21 | awk -v cores="$cores_final" -v map="$map" '{printf "Chain,%s,%s,%s,%s\n",map,cores,$1,$2}' \
    | sed 's/,$//' >> $output_csv
done
