...
output_csv="../../../../../../barrier/Results/barrier_tree.csv"
...
# Run the benchmark test selecting the tree algorithm 6
for map in $maps
do
    ...
    do
        ...
        --mca coll_tuned_barrier_algorithm 6 \
        osu_barrier -x 500 -i $iter -f -z \
        | tail -n 1 | awk -v cores="$cores" -v map="$map" '{printf "Tree,%s,%s,%s\n",map,cores,$1}' >> $output_csv
    done
    ...
    --mca coll_tuned_barrier_algorithm 6 \
    osu_barrier -x 500 -i $iter -f -z \
    | tail -n 1 | awk -v cores="$cores_final" -v map="$map" '{printf "Tree,%s,%s,%s\n",map,cores,$1}' >> $output_csv
done
