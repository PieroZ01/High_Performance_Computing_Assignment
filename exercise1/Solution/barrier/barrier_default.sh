...
#SBATCH --job-name=barrier_default
#SBATCH --nodes=2
...
output_csv="../../../../../../barrier/Results/barrier_default.csv"
...
do
    ...
    do
        ...
        --mca coll_tuned_barrier_algorithm 0 \
        osu_barrier -x 500 -i $iter -f -z \
        | tail -n 1 | awk -v cores="$cores" -v map="$map" '{printf "Default,%s,%s,%s\n",map,cores,$1}' >> $output_csv
    done
    ...
    --mca coll_tuned_barrier_algorithm 0 \
    osu_barrier -x 500 -i $iter -f -z \
    | tail -n 1 | awk -v cores="$cores_final" -v map="$map" '{printf "Default,%s,%s,%s\n",map,cores,$1}' >> $output_csv
done
