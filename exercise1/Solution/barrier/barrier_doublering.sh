...
#SBATCH --job-name=barrier_doublering
#SBATCH --nodes=2
...
output_csv="../../../../../../barrier/Results/barrier_doublering.csv"
...
# Run the benchmark test selecting the double_ring algorithm 2
for map in $maps
do
    ...
    do
        ...
        --mca coll_tuned_barrier_algorithm 2 \
        osu_barrier -x 1000 -i $iter -f -z \
        | tail -n 1 | awk -v cores="$cores" -v map="$map" '{printf "DoubleRing,%s,%s,%s\n",map,cores,$1}' >> $output_csv
    done
    ...
    --mca coll_tuned_barrier_algorithm 2 \
    osu_barrier -x 1000 -i $iter -f -z \
    | tail -n 1 | awk -v cores="$cores_final" -v map="$map" '{printf "DoubleRing,%s,%s,%s\n",map,cores,$1}' >> $output_csv
done
