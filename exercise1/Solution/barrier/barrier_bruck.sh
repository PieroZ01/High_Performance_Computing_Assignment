...
output_csv="../../../../../../barrier/Results/barrier_bruck.csv"
...
# Run the benchmark test selecting the bruck algorithm 4
for map in $maps
do
    ...
    do
        ...
        | tail -n 1 | awk -v cores="$cores" -v map="$map" '{printf "Bruck,%s,%s,%s\n",map,cores,$1}' >> $output_csv
    done
    ...
    --mca coll_tuned_barrier_algorithm 4 \
    osu_barrier -x 1000 -i $iter -f -z \
    | tail -n 1 | awk -v cores="$cores_final" -v map="$map" '{printf "Bruck,%s,%s,%s\n",map,cores,$1}' >> $output_csv
done
